...
def get_one_img_testset(org_img, patch_height, patch_width, stride_height, stride_width):
    
    org_img = np.array(org_img)
    org_img = np.expand_dims(org_img, 0)
    org_img = org_img.transpose(0, 3, 1, 2)
    img = my_PreProc(org_img)

    # img = paint_border_overlap(img, patch_height, patch_width, stride_height, stride_width)
    patches = extract_ordered_overlap(img, patch_height, patch_width, stride_height, stride_width)

    return patches, org_img, img.shape[2], img.shape[3]
